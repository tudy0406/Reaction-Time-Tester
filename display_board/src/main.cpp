...
void update_screen()
{
    ...
    switch (menu_select)
    {
        ...
        case MENU:
            ...
            if (keypad_state == LEFT)
                break;
            if (keypad_state == RIGHT){
        ...
        case CHARACTER:
            if (keypad_state == UP || keypad_state == DOWN)
            {
                ...
                {
                    LCD_GoTo(1, 1);
                    sprintf(buf, "%s", registered_players[options_select + 1]);
                    LCD_WriteText(buf);
                    _delay_ms(100);
                }
                else
        ...
        while (menu_select != IN_GAME)
        {
            ...
            else
            {
                ...
                if (raw < 100)
                {
                    keypad_state = RIGHT;
                    options_select = 0;
                    if (menu_select == DIFFICULTY && options_select < no_registered_players)
                        menu_select = IN_GAME;
                    update_screen();
                }
